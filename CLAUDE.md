...
- Backend vyžaduje na každej ceste `X-Api-Key`. Prehliadač ho nesmie vidieť, preto **nič nevolá `alexapi.sk` priamo**: volá `/api/<cesta>` na našej doméne a kľúč doplní serverová proxy `api/proxy.js` (smerovanie robí rewrite `/api/(.*)` vo `vercel.json`) z env `ALEX_API_KEY` (Vercel → Settings → Environment Variables). Kľúč **nikdy** do JS ani do repa.
